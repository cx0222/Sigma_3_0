...
def _get_(func):
    ...
    if func == 1:
        ...
        outcut = listcs[1]
        window3 = tk.Toplevel()
        window3.title('三元轮换对称不等式运算程序  ver 3.2  SOS分拆结果')
    ...
    if func == 2:
        window4 = tk.Toplevel()
        out0 += sc3_0(exp0)
        window4.title('三元轮换对称不等式运算程序  ver 3.2  Schur分拆结果')
        scroll = tkinter.Scrollbar()
        title4 = tk.Text(window4, width=800, height=500)
        scroll.pack(side=tkinter.RIGHT, fill=tkinter.Y)
        title4.pack(side=tkinter.LEFT, fill=tkinter.Y)
        scroll.config(command=title4.yview)
        title4.config(yscrollcommand=scroll.set)
...
def _catch_(func):
    ...
    if func == 1:
        ...
        out0 += '\n[结果] ' + str(pqr2_6(exp0))
        window5.title('三元轮换对称不等式运算程序  ver 3.2  pqr分拆结果')
    if func == 2:
        if as_ast:
            out0 += '\n[整理] ' + str(normal0(exp0))
        else:
        ...
        listast = ast3_0(exp0)
        out0 += '\n[结果] ' + listast[0]
        window5.title('三元轮换对称不等式运算程序  ver 3.2  差分代换结果')
        cs = tk.Button(window5, text='更多功能', command=lambda: _more_(listast[1]))
        cs.pack()
...
def _more_(list0):
    ...
    window8.title('三元轮换对称不等式运算程序  ver 3.2  进一步运算')
    scroll_8 = Scrollbar(window8)
    scroll_8.pack(side=RIGHT, fill=Y)
    ...
    result4 = auto_prove3_2(num0, list0)
    if result4.__len__() != 0:
        ...
        title8_3 = tk.Label(window8, text='显然')
        title8_3.pack()
    window8.mainloop()
...
def _about_():
    ...
    bg0 = ImageTk.PhotoImage(Image.open('ver3_2.gif'))
...
window0 = tk.Tk()
window0.title('三元轮换对称不等式运算程序  ver 3.2 [20210130]')
window0.geometry('800x420')
window0.resizable(True, True)
title0 = tk.Label(window0,
                  text='\n基于Python 3开发的 三元轮换对称不等式运算程序（测试版）'
                       '\n ver 3.2 [20210130]  cx0222 Jinan, Shandong',
                  font=('Arial', 18))
