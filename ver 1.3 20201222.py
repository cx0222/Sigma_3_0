...
print('三元轮换对称不等式配方程序 for Python')
print('ver 1.3 [20201222]')
print('*cx2020 Jinan, Shandong')
print('Type info() for more information\n'
      'Type instr() for more instructions')
...
def cs1_3(expr):
    ...
    varlistf = sympy.symbols('fvar:{}'.format(fnum))
    s_a, s_b, s_c = 0, 0, 0
    ...
    if judge1(expr0) == 4:
        add1 = sympy.poly(result2, a, b, c)
        add0 = solve(add1.coeff_monomial(b ** 2) - add1.coeff_monomial(c ** 2), varlistf)
        result2 = add1.subs(add0)
    print('Σ (', result2, ') * ( b - c ) ^ 2')
...
def instr():
    ...
    print(' [注意二] 本函数会对于表达式自动求和，即自动对表达式加上 Σ（轮换和），通分后提取分子部分；')
    print(' [注意三] 有多种配方结果时，会参数化表示.')
    print('[功能二] cs1_3(expr)')
    print(' [说　明] 专门对三元齐次（指的是通分以后的分子的次数）轮换对称不等式进行配方.')
    print(' [注意一] expr的类型是字符串，因此需要添加英文引号；')
    print(' [注意二] 本函数会对于表达式自动求和，即自动对表达式加上 Σ（轮换和），通分后提取分子部分；')
    print(' [注意三] 有多种配方结果时，会参数化表示（var[i]）.')
