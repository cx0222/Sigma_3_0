...
def _get_(func):
    ...
    if func == 1:
        ...
        outcut = listcs[1]
        window3 = tk.Toplevel()
        window3.title('三元轮换对称不等式运算程序  ver 3.1  SOS分拆结果')
        ...
        if judge1(entry0.get()) != 3:
            cs01 = tk.Button(window3, text='进一步运算', command=lambda: _further_(outcut))
            cs01.pack()
        window3.mainloop()
    if func == 2:
        window4 = tk.Toplevel()
        out0 += sc3_0(exp0)
        window4.title('三元轮换对称不等式运算程序  ver 3.1  Schur分拆结果')
...
def _catch_(func):
    ...
    if func == 1:
        ...
        out0 += '\n[结果] ' + str(pqr2_6(exp0))
        window.title('三元轮换对称不等式运算程序  ver 3.1  pqr分拆结果')
    if func == 2:
        if as_ast:
            out0 += '\n[整理] ' + str(normal0(exp0))
        else:
        ...
        out0 += '\n[结果] ' + str(ast3_0(exp0))
        window.title('三元轮换对称不等式运算程序  ver 3.1  差分代换结果')
...
def _further_(out):
    ...
    window7.title('三元轮换对称不等式运算程序  ver 3.1  进一步运算')
    ...
    if deg0 == 4:
        ...
        title7_1_1 = tk.Label(window7, text='Ta = ' + str(zero(deg0)))
        ...
        def add_4():
            ...
            out7_4 = sympy.simplify(zero(deg0)) * int(k1) + sympy.simplify(out) * int(k0)
...
def _about_():
    ...
    bg0 = ImageTk.PhotoImage(Image.open('ver3_1.gif'))
