...
print('\n 三元轮换对称不等式运算程序\n')
print(' ver 2.5 [20210113]')
print(' *cx2020 Jinan, Shandong')

a, b, c = sympy.symbols('a, b, c')
s, t, d = sympy.symbols('s, t, d')
as_cs, as_sc, as_pqr, as_ast, ast, coef = False, False, False, False, True, True
...
def pqr2_5(expr):
    ...
    list1, list2, equlist0 = [], dict0(expr), []
    if coef:
        list0 = dict2(expr)[-1]
...
def dict2(expr):
    deg0 = judge1(expr)
    pqrdict = [[] for d in range(0, deg0 + 1)]
    p, q, r = sympy.symbols('p, q, r')
...
def _true_(func):
    global as_cs, as_sc, as_pqr, as_ast, ast, coef
    if func == 1:
        as_cs = not as_cs
    if func == 2:
        as_sc = not as_sc
...
def _about_():
    ...
    bg0 = ImageTk.PhotoImage(Image.open('ver2_5.gif'))
...
cs0.pack(), cs1.pack(), sc0.pack(), sc1.pack(), pqr0.pack(), pqr1.pack(), ast0.pack(), ast1.pack(), ast2.pack()
